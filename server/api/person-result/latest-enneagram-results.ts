...
interface SuccessBody {
  success: true;
  data: EnneagramResultItem[];
}

/**
 * @description 当前登录用户的九型人格测试记录列表（按时间倒序）。
 */
export default defineEventHandler(async (event): Promise<SuccessBody> => {
  const { userId } = await requireAuthUser(event);

  const docs = await UserEnneagramResults.find({ userId }).sort({ createdAt: -1 }).lean().exec();

  const items: EnneagramResultItem[] = docs.map((doc) => ({
    id: String(doc._id),
    userId: doc.userId,
    type: doc.type,
