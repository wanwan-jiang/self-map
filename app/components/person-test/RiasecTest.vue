...
import { RIASEC_TYPE_KEY } from "~/variables/variable";
...
const {
  ...
  goPrev,
  goNext,
} = await usePersonTest(RIASEC_TYPE_KEY);

const handleSubmit = async (): Promise<void> => {
  await usePersonSubmit(RIASEC_TYPE_KEY, answers);
};

useHead({
  title: "SelfMap - RIASEC 测评",
  htmlAttrs: {
    lang: "zh-Hans",
    class: "dark",
  },
