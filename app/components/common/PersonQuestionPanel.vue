  ...
  <div class="w-full max-w-3xl flex flex-col gap-10">
    ...
    >
      <button
        v-for="option in question.options"
        :key="option.id"
        class="group relative flex flex-col p-6 rounded-lg bg-surface-container-low border transition-all duration-300 text-left items-start overflow-hidden active:scale-[0.98]"
        :class="
          isActiveOption(option.id)
            ? 'border-primary/70 bg-surface-container-high'
            : 'border-transparent hover:border-primary/40 hover:bg-surface-container-high'
      ...
      >
        ...
        />
        <span
          class="text-primary-dim font-headline font-black text-3xl mb-4 group-hover:scale-110 transition-transform"
        >
          {{ option.label }}
        </span>
        <p class="text-base font-body font-semibold text-on-surface leading-snug z-10">{{ option.text }}</p>
        <div
          class="mt-6 flex items-center gap-2 text-primary transition-opacity"
          :class="isActiveOption(option.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'"
        >
          <span class="text-[10px] font-bold tracking-tighter">SELECT</span>
